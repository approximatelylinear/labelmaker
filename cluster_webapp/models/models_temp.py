...
def test_esclusterdataframe():
    df_wrapper = ESClusterDataFrame(
        filters={},
        fields=[],
        limit=1000,
    )
    print df_wrapper.session()
...
class ESClusterDataFrame(ClusterDataFrame):
    ...
    fltr_fields = [
        'labels', 'query', 'status', 'community', 'site',
    ]
    base_query = {'content': '*'}
    fld_date = 'date_create'
    ...
    def fmt_choices(self, name, choices, field=None, count=None):
        ...
        return {
            'name': name,
            'choices': [
                # {'count': c['doc_count'], 'key': c['key']} for c in choices
                c['key'] for c in choices
            ]
        }
    ...
    def get_info(self):
        ...
        result = {
            ...
            'filters'   : filters,
            'count'     : count,
            # 'start'     : filter_counts.get('date')['start'],
            # 'end'       : filter_counts.get('date')['end'],
        }
        return result
    ...
    def find_posts(self, filters=None):
        ...
        #   --------------------------------------------------------------
        curr_df = self.session.find_posts_random(query=None, **params)
        # curr_df = self.session.find_posts(query, **params)
        return curr_df
    ...
    def get_filters(self):
        ...
        for k, v in filters.iteritems():
            if k == 'is_labeled':
                ...
                fltr = self.get_content_filter(v)
            elif k == 'status':
            ...
            if fltr:
                for k, v in fltr.iteritems():
                    if k in query:
                        #   Already have this key. Create a conjuntion.
                        query_and = query.setdefault('and', [])
                        query_and.extend([
                            query.pop(k),
                            {k: v}
                        ])
    ...
    def get_content_filter(self, content):
        if content:
            res = {
                "query" : {
                    "match" : {
    ...
    def get_randomize_query(self, query, **params):
        """
        Get query that randomizes results.

        Should be applied after creating all other queries.
        """
...
class ElasticSearchSession(object):
    ...
    def get_query_find_posts_random(self, query, **params):
        import random, string
        if query is None:
            query = "*:*"
        elif isinstance(query, dict):
    ...
    def find_posts_random(self, query, **params):
        import random, string
        if query is None:
            query = "*:*"
        elif isinstance(query, dict):
    ...
    def find_posts(self, query, **params):
        ...
        curr_posts = self.connection.search(
            index=self.db_name, body=body, **params
        )
        curr_posts = curr_posts['hits']['hits']
        new_posts = []
        idx = None
    ...
    def get_filter_counts(self, flds, fltr=None):
        ...
        # print pformat(docs_agg)
        # print pformat(flds_fmt)
        for k in docs_agg:
            # print k
            # print docs_agg[k]
            info = flds_fmt.get(k)
